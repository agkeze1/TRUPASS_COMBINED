...
uint8_t btnState = HIGH;          // last stable state
...
void connectToWiFi();
void beepBuzzer(uint32_t delayMillis);
...
void setup() {
  ...
  pinMode(ENTER_KEY_PIN, INPUT);  // external pull-up already added
  btnState = digitalRead(ENTER_KEY_PIN); // sync initial state
  ...
  if (millis() - lastWiFiCheck >= WIFI_CHECK_INTERVAL) {
    ...
    if (!isWiFiConnected()) {
      ...
      if ((millis() - wifiLostStart) > WIFI_CYCLE_TIMEOUT) {
        ...
        displayTextScreen("Net. conn. failed!");
        delay(2000);
        displayTextScreen("Select Network!");
        wm.setConfigPortalTimeout(CONFIG_PORTAL_TIMEOUT);
      ...
      if (tryingNetworks) {
        ...
        displayTextScreen("Net. connected!");
        delay(2000);
        displayHomeScreen();
        tryingNetworks = false;
        wifiLostStart = 0;
...
void saveNewCredentialToSD(const String &newSSID, const String &newPass) {
  ...
  File file = SD.open("/config/networks.json", FILE_READ);
  if (file) {
    DeserializationError err = deserializeJson(doc, file);
    file.close();
    if (err) {
...
void logSavedNetworks() {
  File file = SD.open("/config/networks.json", FILE_READ);
  if (!file) {
    Serial.println("networks.json not found.");
    return;
  }
  ...
  Serial.println("Current saved networks:");
  for (JsonObject obj : arr) {
    Serial.print("  • SSID: ");
    Serial.print(obj["ssid"].as<String>());
    Serial.print(" | PASS: ");
    Serial.println(obj["password"].as<String>());
  }
}
...
void playAudio(const char *filename) {
  ...
  if (header.sample_rate == 0 || header.channels == 0) {
    Serial.printf("Invalid WAV header in %s\n", filename);
    file.close();
    return;
  }
...
void handlePasskeyScreen(char key) {
  ...
  if (key >= '0' && key <= '9' && passkey.length() < 6) {
    ...
    Serial.println(passkey); // Remove this log
...
void handleEnterKeyPress() {
  int rawState = digitalRead(ENTER_KEY_PIN);

  // detect edge (state change) with debounce
  if (rawState != btnState && millis() - lastEdgeMs > DEBOUNCE_MS) {
    ...
    if (btnState == LOW) {   // button pressed
      ...
      if (passkey.length() == 6) {
        Serial.println("Passkey is 6 digits, validating...");
        beepBuzzer(300);
        validateAccess(passkey);
        passkey = "";
      } else {
        Serial.println("Passkey not complete. Ignoring Enter press.");
      }
    }
  }
}
